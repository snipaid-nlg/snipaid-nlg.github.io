...
> ![Languages labeled in the training data](https://user-images.githubusercontent.com/36483428/230376942-caf86184-eafe-45eb-8755-ce7b3e8471d6.png)
Figure 1: Languages in the ROOTS training data. Source: [https://huggingface.co/bigscience/bloom#languages](https://huggingface.co/bigscience/bloom#languages).
...
Figure 2: Languages in 1% ROOTS sample as labeled from meta-data (ROOTS-1%) and as re-identified using cld3 (ROOTSIDENTIFY-1%). Source: [https://arxiv.org/abs/2211.01786v1](https://arxiv.org/abs/2211.01786v1), truncated at 0.1%.
...
Note: Translation comes at the cost of a slight translation delay. Also translation may occasionally introduce translation errors.
...
Similar rough estimate for BLOOM: 176B parameters in half precision (16 bits = 2 bytes) equals 352 GB RAM. But since some modules are 32-bit, it would actually take somewhat more.

Inference also requires VRAM, so let’s add another ~25% overhead. This puts us at roughly 440 GB of VRAM needed to generate something with these models. 
...
BLOOM + Petals allow multiple individuals and organizations to come together to and join their computing power to . Together news organizations could host a private swarm among trusted entities. Doing so, would enable access to more powerful language models than any one entity could afford to run on their own.
