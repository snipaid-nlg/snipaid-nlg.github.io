...
Even though MTF helps LLMs generalize to unseen tasks & languages, we find that the German capabilities of BLOOMZ models are lacking. In English, however, we find that the BLOOMZ models are capable of generating news snippets when provided with a news article and a simple prompt for the task.
